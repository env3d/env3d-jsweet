...
if (
  localStorage &&
  (!localStorage.getItem("code") || localStorage.getItem("code").length == 0)
) {
  let code = {
    "Room.java": [
      `public class Room {`,
  ...
  transpile(finalCode)
    ...
    .catch((e) => {
      ...
      for (let i = 0; i < numOfErrors; i++) {
        ...
        if (errorDescArr.length >= 3) {
          ...
          for (j in errorDescArr) {
            if (errorDescArrLength[j].split(" ").includes("Line")) {
              errorLineNumber = errorDescArrLength[j]
                .split(":")[0]
                .split(" ")[1];
            }
            if (errorDescArrClass[j].split(" ").includes("location:")) {
              errorClassName = errorDescArrClass[j].split(" ")[
                errorDescArrClass[j].split(" ").indexOf("class") + 1
              ];
          ...
          let marker = editor.session.addMarker(
            new Range(errorLineNumber - 1, 0, errorLineNumber, 0),
            "highlightError",
            "line"
          );
          markers.push(marker);
        ...
        message.innerHTML = `<b>${Date()
          .toString()
          .slice(0, 24)}:</b> <span style="color:red"><i><b>${
          errors[i]
        }</b></i></span> <br>`;
...
function addLightAfterTranspile() {
  setTimeout(function () {
    window.game.env.scene.add(
      new f.contentWindow.THREE.HemisphereLight(0x7f7f7f, 0x000000, 4)
    );
  }, 2000);
}
...
window.addEventListener("message", function (evt) {
  ...
  if (
    ...
    evt.data &&
    evt.data != "applet onload hook"
  ) {
    let message = document.createElement("div");
    message.innerHTML = `<b>${Date().toString().slice(0, 24)}:</b> ${
      evt.data
    } <br>`;
    document.getElementById("console").prepend(message);
  }
...
function deleteFile(file) {
  ...
  document.getElementById("tabs").removeChild(document.getElementById(file));
  saveFile();
...
window.addEventListener("load", function (evt) {
  ...
  if (inputCode) {
    ...
    localStorage.setItem(
      "code",
      atob(inputCode.replace(/\./g, "+").replace(/-/g, "=").replace(/_/g, "/"))
    );
  ...
  editor.on("change", () => {
    ...
    status.innerHTML =
      "source changed, click run (command-enter) to see changes in 3D window";
...
let init = () => {
  ...
  else consoleArrows.className = "fa fa-angle-double-up";
  //resizeEditor();
...
const expungeThings = () => {
  ...
  document.getElementById("utilItems").style.display = "none";
  authForm.style.display = "block";
};

const bringBackThings = () => {
  if (user !== null) {
    if (user.emailVerified) {
      document.getElementById("run").style.display = "block";
      document.getElementById("editor").style.display = "block";
      document.getElementById("tabs").style.visibility = "inherit";
      document.getElementById("navItems").style.display = "block";
      document.getElementById("utilItems").style.display = "block";
    }
  }
